...
CREATE TABLE IF NOT EXISTS `qsystem`.`clients` (
  ...
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_clients_service_id_services_id`
    FOREIGN KEY (`service_id`)
    REFERENCES `qsystem`.`services` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_clients_user_id_users_id`
    FOREIGN KEY (`user_id`)
...
CREATE INDEX `idx_clients_service_id_services_id` ON `qsystem`.`clients` (`service_id` ASC);

CREATE INDEX `idx_clients_user_id_users_id` ON `qsystem`.`clients` (`user_id` ASC);
...
CREATE TABLE IF NOT EXISTS `qsystem`.`statistic` (
  ...
  CONSTRAINT `fk_work_user_id_users_id`
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_work_client_id_clients_id`
...
CREATE INDEX `idx_work_client_id_clients_id` ON `qsystem`.`statistic` (`client_id` ASC);
...
START TRANSACTION;
USE `qsystem`;
INSERT INTO `qsystem`.`net` (`id`, `server_port`, `web_server_port`, `client_port`, `finish_time`, `start_time`, `version`, `first_number`, `last_number`, `numering`, `point`, `sound`, `branch_id`, `sky_server_url`, `zone_board_serv_addr`, `zone_board_serv_port`, `voice`, `black_time`, `limit_recall`, `button_free_design`, `ext_priority`) VALUES (1, 3128, 8088, 3129, '18:00:00', '07:45:00', '7', 1, 999, 0, 0, 1, 113, 'http://localhost:8080/qskyapi/customer_events?wsdl', '127.0.0.1', 3127, 0, 0, 0, 0, 0);
